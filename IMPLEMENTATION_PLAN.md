...
## 🟡 Epic 7: Billing & Subscription Module (Next Priority)
- [ ] **7.1 Stripe Integration**: Implement `StripeService` and webhook handling for payment processing.
- [ ] **7.2 Subscription UI**: Build the "Subscription" tab in the Admin Dashboard with tiered pricing cards.
- [ ] **7.3 Feature Gating**: Implement backend logic to restrict Knowledge Base uploads and AI features based on user tier.
- [ ] **7.4 Checkout Flow**: Integrate Stripe Checkout for seamless upgrades from Free to Pro tiers.

## 🟡 Epic 8: Quality Assurance & Testing
- [ ] **8.1 Data Import Testing**: Comprehensive unit and integration tests for sales/customer import logic.
- [ ] **8.2 Security Integration Tests**: Verify CORS restrictions and JWT safety failure states.
- [ ] **8.3 End-to-End Widget Test**: Automated flow from widget chat to backend RAG response.
- [ ] **8.4 Security-First Code Review**: Comprehensive "Web-First" audit of frontend/API communication and data sanitization.
- [ ] **8.5 Cloud-First Architecture Review**: Evaluate system portability (Docker, secrets, statelessness) for Azure/AWS deployment vs. local-first bias.
- [ ] **8.6 Secret Sanitization & Hardening**: Audit codebase to ensure zero hardcoded secrets/sensitive values; verify `.gitignore` integrity for all env files.
...
- [ ] **9.4 Online Demo (Cloud-Ready)**: Deploy a "free demo" version using a HuggingFace/OpenRouter free model for zero-cost evaluation.
